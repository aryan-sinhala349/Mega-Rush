...
#define MR_ASSERT(x, ...) { if(!(x)) { MR_ERROR("Assertion Failed: {0}", __VA_ARGS__); MR_DEBUGBREAK(); } }
#define MR_CORE_ASSERT(x, ...) { if(!(x)) { MR_CORE_ERROR("Assertion Failed: {0}", __VA_ARGS__); MR_DEBUGBREAK(); } }
#else
#define MR_ASSERT(x, ...)
...
#define MR_BIND_EVENT_FN(fn) [this](auto&&... args) -> decltype(auto) { return this->fn(std::forward<decltype(args)>(args)...); }
